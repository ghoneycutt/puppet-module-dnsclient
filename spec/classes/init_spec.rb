...
describe 'dnsclient' do
  ...
  context 'when run on Linux systems' do
    let :facts do
      { :kernel => 'Linux' }
    end
...
nameserver 8.8.4.4
  ...
  context 'When run on Windows systems' do
    let :facts do
      { :kernel => 'windows' }
    end
    ...
    context 'When run with default params' do

      it { should contain_class('dnsclient') }

      it {
        should contain_exec('set dns').with({
          'command'  => "\$interface = find-netroute -RemoteIPAddress 8.8.8.8 | select -expand 'InterfaceAlias'; set-dnsclientserveraddress -interfacealias \$interface -serveraddress {8.8.8.8,8.8.4.4}",
          'provider' => 'powershell',
          'unless'   => "\$interface = find-netroute -RemoteIPAddress 8.8.8.8 | select -expand 'InterfaceAlias'; \$currentdns = (get-dnsclientserveraddress -interfacealias \$interface | select -expand 'ServerAddresses') -join ','; if (\$currentdns -ne '8.8.8.8,8.8.4.4') { exit 1 }",
        })
      }
    end
  end
  
  context 'When run on an unsupported operating system' do
    let :facts do
      { :kernel => 'darwin' }
    end
    it 'should fail' do
      expect {
        should raise_error("Unsupported operating system $::kernel")
      }
    end
  end
end
